		...
		GetAmazonOrderEmailsSummaryResponse>
{
	...
	private Email SummaryEmail(Dictionary<Email, List<string>> emailsWithOrders)
	{
		return new Email
		{
			...
			Subject = $"Summary of Amazon Orders Emails since: {emailsWithOrders.Min(email => email.Key.Sent):yyyy-MM-dd}"
		};
	}
	...
	private string HtmlBody(Dictionary<Email, List<string>> emailsWithOrders)
	{
		...
		Dictionary<Guid, Account> accounts = _accounts.ToDictionary(e => e.Id, account => account);
		...
		{
			List<KeyValuePair<Email, List<string>>> orderEmails = emailsWithOrders
				.Where(e => e.Value.Contains(order))
				.OrderByDescending(e => e.Key.Sent)
				.ToList();

			htmlBody.AppendLine($"<h2>{order} <i>({accounts[orderEmails.First().Key.AccountId].Name})</i>:</h2>");
