...
public struct UniqueId : IComparable<UniqueId>, IEquatable<UniqueId>
{
	...
	public uint Validity => this.validity;
	...
	public int CompareTo(UniqueId other) => this.Id.CompareTo(other.Id);
	...
	public bool Equals(UniqueId other) => (int)other.Id == (int)this.Id;
	...
	public static bool operator ==(UniqueId uid1, UniqueId uid2) => (int)uid1.Id == (int)uid2.Id;
	...
	/// <param name="uid1">The first unique id to compare.</param>
	/// <param name="uid2">The second unique id to compare.</param>
	public static bool operator >(UniqueId uid1, UniqueId uid2) => uid1.Id > uid2.Id;
	...
	public override bool Equals(object obj) => obj is UniqueId uniqueId && (int)uniqueId.Id == (int)this.Id;
	...
	public static bool TryParse(string token, uint validity, out UniqueId uid)
	{
		...
		uint result;
		if (!uint.TryParse(token, NumberStyles.None, (IFormatProvider)CultureInfo.InvariantCulture, out result) ||
		    result == 0U)
		{
	...
	/// <paramref name="token" /> is <c>null</c>.
	/// </exception>
	public static bool TryParse(string token, out UniqueId uid) => UniqueId.TryParse(token, 0U, out uid);
	...
	/// </exception>
	public static UniqueId Parse(string token, uint validity) => new UniqueId(validity,
		uint.Parse(token, NumberStyles.None, (IFormatProvider)CultureInfo.InvariantCulture));
	...
	public static UniqueId Parse(string token) =>
		new UniqueId(uint.Parse(token, NumberStyles.None, (IFormatProvider)CultureInfo.InvariantCulture));
}
